...
Uses `claude-opus-4-6` with prompt caching. Outputs JSON files to `pge-project/workflows/`. Prompts are in `pge-project/prompts/{planner,generator,evaluator}.md`.

The Claude Code agents `legacy-to-mcp-planner`, `legacy-to-mcp-generator`, and `legacy-to-mcp-evaluator` mirror this flow for the legacy-to-MCP demos.
